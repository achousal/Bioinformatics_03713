...
# Define top-level output directories
...
# This replaces the old Step 2 which ran cross_species.sh
...
# Function to safely calculate percentages (can be reused)
calc_pct_step2() {
...
# Note the variable names match the previous script's expectations for the R scripts
...
# The R script seems to expect 'conserved' and 'not conserved' based on *human liver* vs mouse liver/any
# and *human pancreas* vs mouse pancreas/any. We use the corresponding files generated above.
...
Rscript CHIPseeker_Cross_tissue.R "$HUMAN_SHARED" "$HUMAN_LIVER_SPECIFIC" "$HUMAN_PANCREAS_SPECIFIC" \
                                  "$MOUSE_SHARED" "$MOUSE_LIVER_SPECIFIC" "$MOUSE_PANCREAS_SPECIFIC" \
                                  --output_dir "$CHIPSEEKER_OUTPUT_DIR/cross_tissue_results" # Assuming R script accepts an output dir
...
Rscript CHIPseeker_Cross_species.R "$HUMAN_LIVER_CONSERVED" "$HUMAN_LIVER_NOT_CONSERVED" "$MOUSE_LIVER_NOT_CONSERVED" \
                                   ...
                                   --output_dir "$CHIPSEEKER_OUTPUT_DIR/cross_species_results" # Assuming R script accepts an output dir
